    ...
    <motion.div ref={ref} className="flex flex-col ml-20 my-12" initial="hidden" animate={inView ? 'visible' : 'hidden'} variants={variants}>
      ...
      <div className="flex overflow-x-scroll" style={{ width: '80rem' }}> 
        {tips.map((tip, index) => (
          <motion.div
            key={index}
            className="flex-shrink-0 w-64 mx-2 bg-white rounded shadow-md flex flex-col justify-between cursor-pointer"
            whileHover={{ scale: 1.05 }}
            onClick={() => navigate(`/tips/${tip.id}`)}
          >
            <img src={tip.imageUrl || "https://via.placeholder.com/150"} alt="Tip" className="w-full h-32 object-cover rounded-t" /> 
